...
public class PaymentPredictionService {
    ...
    private Map<LocalDate, DailyStats> buildDailySeries(List<AdminDashboardController.PaymentRow> validRows) {
        ...
        for (AdminDashboardController.PaymentRow row : validRows) {
            grouped.computeIfAbsent(row.getActualDate(), ignored -> new DailyStats())
                    .add(row.getMontant());
        }
        ...
        LocalDate end = LocalDate.now().isAfter(grouped.keySet().stream().max(LocalDate::compareTo).orElse(LocalDate.now()))
                ? LocalDate.now()
                : grouped.keySet().stream().max(LocalDate::compareTo).orElse(LocalDate.now());
    ...
    private double predictRevenueForDate(Map<LocalDate, DailyStats> dailySeries, LocalDate targetDate) {
        List<Double> recentValues = extractRecentRevenues(dailySeries, 7);
        double recentAverage = averageDouble(recentValues);
    ...
    private int predictOrdersForDate(Map<LocalDate, DailyStats> dailySeries, LocalDate targetDate) {
        List<Integer> recentValues = extractRecentCounts(dailySeries, 7);
        double recentAverage = averageInt(recentValues);
